...
def main() -> None:
    ...
    with webdriver.Chrome(options) as driver:
        ...
        while has_next_page:
            ...
            while (
                page_screens_count < constants.MAX_SCREENS_COUNT
                and not is_last_photo
            ):
                ...
                if not screens_count == 1:
                    if (
                        files_manager.compare_pngs(screens_count - 1, screens_count)
                        is True
                    ):
                ...
                page_driver.scroll_down(
                    screen_height * constants.PAGE_SCROLL_PERCENT
                )
            ...
            if page_driver.is_next_page_button_found():
                ...
                page_driver.open_new_tab(
                    url=next_page_url(base_url=url, page_number=pages_count)
                )
                prepare_page()
    ...
    if not files_manager.is_exist(constants.DOCX_PATH.format(url)):
...
if __name__ == '__main__':
    try:
        ...
        if isinstance(error, WebDriverException):
            user_interface.ErrorUI(
                'Введена неправильная ссылка или нет доступа к сети'
            )
